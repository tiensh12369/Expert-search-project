...
for i in range(len(key_querys)):
    ...
    for mng_one in mng_dict :
        print(mng_one)
        ori_inst = mng_dict[mng_one]['oriInst']
        exi_inst = mng_dict[mng_one]['inst']
        mng_name = mng_dict[mng_one]['name']
        paper = mng_dict[mng_one]['papers']

        Answer = {'fid': f_id, 'keyId': keyid, 'name' : mng_name , 'inst': '', site[i] : {'inst' :exi_inst, 'A_id': [mng_one], 'papers' : paper, 'oriInst' : ori_inst} }
            ...
            while flag :
                ...
                for key in temp.keys() : # 사이트 돌면서
                    if key != 'name' and key != 'keyId' and key != 'fid' and key != 'inst': 
                        ...
                        if len(exi_inst) >= len(temp[key]['inst']):
                            src = temp[key]['inst']
                            tgt = exi_inst

                        elif len(exi_inst) < len(temp[key]['inst']):
                            src = exi_inst
                            tgt = temp[key]['inst']

                        if key == site[i] :# 사이트가 동일할때
                            if temp[key]['inst'] == exi_inst or (src != "" and src in tgt) :  # 소속 같을때
                            ...
                            if temp[key]['inst'] == exi_inst  or (src != "" and src in tgt):  # 소속 같을때
                                Answer_dict[tempName][site[i]] =  {'inst' : exi_inst, 'A_id': [mng_one], 'papers' : paper, 'oriInst' : ori_inst}
