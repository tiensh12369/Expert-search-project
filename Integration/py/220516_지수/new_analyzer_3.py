...
class factor_integration(threading.Thread):
    ...
    def __init__(self, start, num_data, fid, keyID, stop_words):
        ...
        if self.fid == 0:
            ...
            #self.rank_inst.insert_one({'keyId' : self.keyId, 'Insts' : None})
            #self.rank_inst.insert_one({'keyId' : self.keyId, 'Insts.name' : self.base_insts.keys(), 'Insts.cnt' : self.base_insts.values()})
    ...
    def run(self):
        ...
        for i in range(allPage):
            ...
            accuracy = self.acc(keywords, contBit, querykey) #, self.stop_words, self.komoran)
            ...
            #    print(e)
            #    continue
            # print(real_final_last_data)
            # break
            # print([_['factor']['acc'] for _ in real_final_last_data])
            self.ID['test'].insert_many(real_final_last_data)
        # print(insts_to_update_dict)
    ...
    def acc(self, keywords, contBit, querykey): #, stop_words, komoran):
        rtv = contBit.copy()
    ...
    for sublist in keywords :
        for item in sublist :
            if item is not None and item != 'None' and item != "" and isinstance(item, str) :
                ...
                # added code
                # flat_list.append(" ".join([ _ for _ in item.split() if _ not in stop_words]))
                # flat_list.append(" ".join([word for word, pos in nltk.pos_tag(nltk.word_tokenize(item)) if (pos == 'NN' or pos == 'NNP' or pos == 'NNS' or pos == 'NNPS')]))
                flat_list.append(
                    " ".join(
                        [
